...
public final class AiImageHandler implements ImageHandler {
    ...
    public void loadImage(Context context, String url, Callback<Drawable> callback) {
        ...
        Glide.with(context).load(url).transition(DrawableTransitionOptions.withCrossFade()).apply(new RequestOptions().override(defaultW)).into(new SimpleTarget<Drawable>() {
            @Override
