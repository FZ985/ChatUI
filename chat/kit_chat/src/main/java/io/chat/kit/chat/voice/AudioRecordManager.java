package io.chat.kit.chat.voice;
...
public class AudioRecordManager implements Handler.Callback {
    ...
    private OnRecordResultCallback mCallback;

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    private AudioRecordManager() {
        JLog.e(TAG, "AudioRecordManager");
    ...
    public final boolean handleMessage(android.os.Message msg) {
        JLog.e(TAG, "handleMessage " + msg.what);
        switch (msg.what) {
    ...
    private void initView(View root) {
        ...
        View view = inflater.inflate(R.layout.voice_record_popup, null);
        mStateIV = view.findViewById(R.id.audio_state_image);
        mStateTV = view.findViewById(R.id.audio_state_text);
        mTimerTV = view.findViewById(R.id.audio_timer);
        mRecordWindow = new PopupWindow(
                view,
    ...
    private void setTimeoutView(int counter) {
        if (counter > 0) {
            if (mRecordWindow != null) {
                ...
                mStateTV.setText(R.string.chat_voice_rec);
            ...
            if (mRecordWindow != null) {
                ...
                mStateIV.setImageResource(R.drawable.voice_volume_warning);
                mStateTV.setText(R.string.chat_voice_too_long);
                mStateTV.setBackgroundResource(android.R.color.transparent);
    ...
    private void setRecordingView() {
        JLog.e(TAG, "setRecordingView");

        if (mRecordWindow != null) {
            mStateIV.setVisibility(View.VISIBLE);
            mStateIV.setImageResource(R.drawable.voice_volume_1);
            mStateTV.setVisibility(View.VISIBLE);
            mStateTV.setText(R.string.chat_voice_rec);
            mStateTV.setBackgroundResource(android.R.color.transparent);
            mTimerTV.setVisibility(View.GONE);
    ...
    private void setCancelView() {
        JLog.e(TAG, "setCancelView");
        if (mRecordWindow != null) {
            mTimerTV.setVisibility(View.GONE);
            mStateIV.setVisibility(View.VISIBLE);
            mStateIV.setImageResource(R.drawable.voice_volume_cancel);
            mStateTV.setVisibility(View.VISIBLE);
            mStateTV.setText(R.string.chat_voice_cancel);
            mStateTV.setBackgroundResource(R.drawable.voice_cancel_background);
        }
    }
    ...
    private void setCallStateChangeListener() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            try {
                ...
                JLog.e(TAG, "AudioRecordManager", e);
            }
        }
    }

    private void destroyView() {
        JLog.e(TAG, "destroyView");
        if (mRecordWindow != null) {
    ...
    public void startRecord(View rootView) {
        ...
        this.mAfChangeListener = new AudioManager.OnAudioFocusChangeListener() {
            public void onAudioFocusChange(int focusChange) {
                JLog.e(TAG, "OnAudioFocusChangeListener " + focusChange);
...
//        }
    ...
    public void stopRecord(OnRecordResultCallback callback) {
    ...
    private void startRec() {
        ...
        try {
            ...
            mMediaRecorder.setOnErrorListener((mr, what, extra) -> JLog.e(
                    TAG,
...
//            }
            ...
            if (!savePath.canWrite()) {
                ...
                if (!result) {
                    String msg = new StringBuilder(savePath.getPath())
    ...
    private void stopRec() {
        ...
        try {
            ...
            JLog.e(TAG, "stopRec", e);
        }
    }

    private void deleteAudioFile() {
        JLog.e(TAG, "deleteAudioFile");
        ...
        if (mAudioPath != null) {
            ...
            if (file.exists()) {
                ...
                if (!deleteResult) {
                    JLog.e(TAG, "deleteAudioFile delete file failed. path :" + mAudioPath.getPath());
                }
            }
        }
    }
    ...
    private void sendAudioFile() {
        JLog.e(TAG, "发送语音 sendAudioFile path = " + mAudioPath);
        if (mAudioPath != null) {
            File file = new File(mAudioPath.getPath());
            if (!file.exists() || file.length() == 0) {
                JLog.e(TAG, "sendAudioFile fail cause of file length 0 or audio permission denied");
                return;
            }
            int duration = (int) (SystemClock.elapsedRealtime() - smStartRecTime) / 1000;
            if (mCallback != null) {
                mCallback.onRecordResult(new AudioDataBean(mAudioPath.getPath(), duration));
            }
//            UploadModel.get().uploadFile(file, url -> {
//                HQVoiceMessage hqVoiceMessage = HQVoiceMessage.obtain(url, mAudioPath, Math.min(duration, RECORD_INTERVAL));
//                Message message = Message.obtain(userCall.getTargetUser(), mConversationType, MessageType.VOICE, hqVoiceMessage);
//                MessageManager.getInstance().sendMessage(message, null);
//            }, () -> {
//            }, percent -> {
//            });
        }
    ...
    private void audioDBChanged() {
        if (mMediaRecorder != null) {
            ...
            try {
                ...
                JLog.e(TAG, "audioDBChanged IllegalStateException");
            }
            switch (db / 5) {
                case 0:
                    mStateIV.setImageResource(R.drawable.voice_volume_1);
                    break;
                case 1:
                    mStateIV.setImageResource(R.drawable.voice_volume_2);
                    break;
                case 2:
                    mStateIV.setImageResource(R.drawable.voice_volume_3);
                    break;
                case 3:
                    mStateIV.setImageResource(R.drawable.voice_volume_4);
                    break;
                case 4:
                    mStateIV.setImageResource(R.drawable.voice_volume_5);
                    break;
                case 5:
                default:
                    mStateIV.setImageResource(R.drawable.voice_volume_6);
                    break;
            }
        }
    }
    ...
    private void muteAudioFocus(AudioManager audioManager, boolean bMute) {
        if (Build.VERSION.SDK_INT < 8) {
            ...
            JLog.e(TAG, "muteAudioFocus Android 2.1 and below can not stop music");
            return;
        }
        if (audioManager == null) {
            JLog.e(TAG, "audioManager is null");
            return;
        }
        if (bMute) {
    ...
    class IdleState extends IAudioState {
        ...
        void handleMessage(AudioStateMessage msg) {
            JLog.e(TAG, "IdleState handleMessage : " + msg.what);
            switch (msg.what) {
    ...
    class RecordState extends IAudioState {
        @Override
        void handleMessage(AudioStateMessage msg) {
            JLog.e(TAG, getClass().getSimpleName() + " handleMessage : " + msg.what);
            switch (msg.what) {
                case AUDIO_RECORD_EVENT_SAMPLING:
                    audioDBChanged();
                ...
                case AUDIO_RECORD_EVENT_RELEASE:
                    ...
                    if (checked && !activityFinished) {
                        mStateIV.setImageResource(R.drawable.voice_volume_warning);
                        mStateTV.setText(R.string.chat_voice_short);
                        mHandler.removeMessages(AUDIO_RECORD_EVENT_SAMPLING);
                    }
    ...
    class SendingState extends IAudioState {
        ...
        void handleMessage(AudioStateMessage message) {
            JLog.e(TAG, "SendingState handleMessage " + message.what);
            switch (message.what) {
    ...
    abstract static class IAudioState {
        void enter() {
        }
    ...
    static class AudioStateMessage {
        public int what;
        public Object obj;
    ...
    public interface OnRecordResultCallback {
        ...
        void onRecordResult(AudioDataBean result);
    }
}
