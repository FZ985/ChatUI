...
import io.ai.chat.model.SearchProvider
import io.ai.chat.model.resolvedSearchProvider
import io.ai.chat.utils.ALog
import io.ai.chat.utils.optIntOrNull
...
//            .put(buildShellDefinition())
    ...
    fun toolDefinitions(settings: ApiSettings = ApiSettings()): JSONArray {
        ...
        when (searchProvider) {
            SearchProvider.Searxng -> definitions.put(
                JSONObject(
                    cachedSearxngWebSearchToolDefinition.toString()
                )
            )

            SearchProvider.Tavily -> definitions.put(JSONObject(cachedTavilyWebSearchToolDefinition.toString()))
            SearchProvider.Anysearch -> definitions.put(
                JSONObject(
                    cachedAnysearchWebSearchToolDefinition.toString()
                )
            )
...
//                "toolCallId" to ALog.shortId(toolCall.id),
...
//                    ALog.w(Component, ALog.event("收到不支持的工具", "tool" to toolCall.name))
//                    put("ok", false)
...
//    }
    ...
    fun activatePendingShellConfirmation(confirmationId: String): Boolean {
        ...
        val confirmation = pendingShellConfirmations[confirmationId] ?: run {
            ALog.w(
                Component,
                ALog.event("Shell 确认令牌不存在", "confirmationId" to ALog.shortId(confirmationId))
            )
            return false
        }
        if (now - confirmation.createdAtMs > ShellConfirmationTtlMs) {
            ...
            ALog.w(
                Component,
                ALog.event("Shell 确认令牌已过期", "confirmationId" to ALog.shortId(confirmationId))
            )
            return false
        }
        ...
        ALog.i(
            ...
            ALog.event(
                "Shell 确认令牌已激活",
                "confirmationId" to ALog.shortId(confirmationId),
    ...
    fun isShellConfirmationRequest(message: AIChatMessage): Boolean {
    ...
    internal fun readFile(arguments: String): String {
        ...
        ALog.d(
            ...
            ALog.event(
                "读取文件工具完成",
                "pathHash" to ALog.fingerprint(targetFile.path),
                "size" to fileSize,
    ...
    private fun listPath(arguments: String): String {
        ...
        return if (target.isDirectory) {
            ...
            ALog.d(
                ...
                ALog.event(
                    "列出目录工具完成",
                    "pathHash" to ALog.fingerprint(target.path),
    ...
    internal fun getCurrentTime(): String {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val instantDate = Date(timeMs)
            val calendar = Calendar.getInstance()
            calendar.timeInMillis = timeMs

            // ISO_OFFSET_DATE_TIME 格式示例：2026‑08‑21T16:22:33.123+08:00
            val isoFormatter = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSXXX", Locale.US)
            isoFormatter.timeZone = calendar.timeZone
            val currentTimeStr = isoFormatter.format(instantDate)

            val tz = calendar.timeZone
            val zoneId = tz.id

            // 计算UTC offset，格式 +08:00
            val offsetMillis = tz.getOffset(timeMs)
            val offsetHour = offsetMillis / (1000 * 60 * 60)
            val offsetMinute = Math.abs(offsetMillis / (1000 * 60)) % 60
            val utcOffsetStr = String.format(Locale.US, "%+02d:%02d", offsetHour, offsetMinute)

            return JSONObject().apply {
                put("ok", true)
                put("current_time", currentTimeStr)
                put("timezone", zoneId)
                put("utc_offset", utcOffsetStr)
                put("unix_timestamp_ms", timeMs)
            }.toString(2)
        }
    ...
    private fun fetchUrlWithRedirects(
        initialUrl: URL,
        outputFormat: String,
        maxChars: Int
    ): String {
        ...
        repeat(MaxFetchRedirects + 1) { redirectCount ->
            ...
            try {
                    ...
                    if (location.isNullOrBlank()) {
                        ALog.w(
                            ...
                            ALog.event(
                                ...
                                "origin" to ALog.safeUrlOrigin(currentUrl.toString()),
                ...
                val responseBody =
                    readHttpResponseBody(connection, statusCode, MaxFetchResponseBytes)
                if (statusCode !in 200..299) {
                    ...
                    return JSONObject().apply {
                        ...
                        put(
                            "content",
                            decodeResponseBody(responseBody.bytes, contentType).take(maxChars)
                        )
                    }.toString(2)
                }
                ...
                return JSONObject().apply {
                    ...
                    put(
                        "content",
                        if (truncatedContent) formattedContent.take(maxChars) else formattedContent
                    )
                }.toString(2)
            } finally {
    ...
    private fun fetchWebView(context: Context, arguments: String): String {
        ...
        ALog.i(
            ...
            ALog.event(
                "开始 fetch_webview",
                "origin" to ALog.safeUrlOrigin(options.url.toString()),
        ...
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ALog.w(Component, "fetch_webview 被拒绝：不能在主线程同步执行。")
            return JSONObject().apply {
                put("ok", false)
        ...
        fun finishError(message: String, finalUrl: String = webView.url ?: options.url.toString()) {
            ALog.w(
                ...
                ALog.event(
                    "fetch_webview 加载失败",
                    "origin" to ALog.safeUrlOrigin(options.url.toString()),
                    "finalOrigin" to ALog.safeUrlOrigin(finalUrl),
        ...
        fun extractPage() {
            ...
            runCatching {
                webView.evaluateJavascript(script) { value ->
                    ...
                    runCatching {
                        val page =
                            parseWebViewExtractedPage(value, webView.url ?: options.url.toString())
    ...
    private fun parseWebViewExtractedPage(
        value: String?,
        fallbackUrl: String
    ): WebViewExtractedPage {
    ...
    private fun formatWebViewFetchResult(
        options: WebViewFetchOptions,
        page: WebViewExtractedPage
    ): String {
        ...
        ALog.i(
            ...
            ALog.event(
                "fetch_webview 完成",
                "origin" to ALog.safeUrlOrigin(options.url.toString()),
                "finalOrigin" to ALog.safeUrlOrigin(page.finalUrl),
                "outputFormat" to options.outputFormat,
                "converted" to (markdownConversion?.converted ?: false),
        ...
        return JSONObject().apply {
            ...
            put(
                "content",
                if (truncatedContent) formattedContent.take(options.maxChars) else formattedContent
            )
        }.toString(2)
    }
    ...
    private fun searchWithSearxng(settings: ApiSettings, arguments: String): String {
        ...
        ALog.i(
            ...
            ALog.event(
                "开始 web_search",
                "origin" to ALog.safeUrlOrigin(searxngBaseUrl),
                "queryLength" to query.length,
                "queryHash" to ALog.fingerprint(query),
                "page" to page,
                "limit" to limit
            )
        )
        ...
        return try {
            ...
            formatSearchResults(
                ...
                val returned =
                    runCatching { JSONObject(output).optInt("returned", 0) }.getOrDefault(0)
                ALog.i(
                    Component,
                    ALog.event(
                        "web_search 完成",
    ...
    private fun searchWithTavily(settings: ApiSettings, arguments: String): String {
        ...
        ALog.i(
            ...
            ALog.event(
                "开始 web_search",
                "provider" to "Tavily",
                "origin" to ALog.safeUrlOrigin(searchUrl.toString()),
                "queryLength" to query.length,
                "queryHash" to ALog.fingerprint(query),
                "limit" to limit
            )
    ...
    private fun putOptionalTavilyEnum(
        ...
        val value = payload.optString(key).trim().lowercase(Locale.ROOT).takeIf { it.isNotEmpty() }
            ?: return
    ...
    private fun putOptionalTavilyInt(
        payload: JSONObject,
        target: JSONObject,
        key: String,
        min: Int,
    ...
    private fun readHttpResponseText(
        connection: HttpURLConnection,
        statusCode: Int,
        maxChars: Int
    ): String {
    ...
    private fun formatSearchResults(
        ...
        for (index in 0 until rawResults.length()) {
            ...
            results.put(
                JSONObject().apply {
                    ...
                    item.optString("engine").trim().takeIf { it.isNotEmpty() }
                        ?.let { put("engine", it) }
                    item.optString("category").trim().takeIf { it.isNotEmpty() }
                        ?.let { put("category", it) }
        ...
        return JSONObject().apply {
            ...
            root.optJSONArray("suggestions")?.takeIf { it.length() > 0 }
                ?.let { put("suggestions", it) }
    ...
    private fun formatTavilySearchResults(query: String, limit: Int, root: JSONObject): String {
        ...
        for (index in 0 until rawResults.length()) {
            ...
            results.put(
                JSONObject().apply {
                    ...
                    item.optString("raw_content").trim().takeIf { it.isNotEmpty() }
                        ?.let { rawContent ->
                            put("raw_content", truncateSearchRawContent(rawContent))
                        }
                    item.optString("favicon").trim().takeIf { it.isNotEmpty() }
                        ?.let { put("favicon", it) }
                    item.optJSONArray("images")?.takeIf { it.length() > 0 }
                        ?.let { put("images", it) }
                }
            )
        }
        ...
        return JSONObject().apply {
            ...
            root.optString("response_time").trim().takeIf { it.isNotEmpty() }
                ?.let { put("response_time", it) }
    ...
    private fun formatAnysearchResults(query: String, limit: Int, root: JSONObject): String {
            ...
            return JSONObject().apply {
                ...
                put(
                    ...
                    root.optString("message")
                        .ifBlank { "AnySearch 搜索失败，错误码 $responseCode。" })
                root.optString("request_id").takeIf { it.isNotBlank() }
                    ?.let { put("request_id", it) }
            }.toString(2)
        }
        ...
        return JSONObject().apply {
            ...
            put(
                "available",
                data.optJSONObject("metadata")?.optInt("total_results", rawResults.length())
                    ?: rawResults.length()
            )
            put("has_more", rawResults.length() > results.length() || skipped > 0)
            put("results", results)
...
//        context: Context,
//        arguments: String,
//        onShellProcessStarted: ((String, Boolean) -> Unit)?
//    ): String {
//        val payload = JSONObject(arguments)
...
//        if (risk != null && !consumeShellConfirmation(
//                confirmationId,
//                command,
//                workDir,
//                background
...
//                    "confirmationId" to ALog.shortId(id),
//                    "commandHash" to ALog.fingerprint(command),
...
//        val id = ShellProcessManager.start(
//            command,
//            effectiveWorkDir,
//            BusyBoxManager.environmentOverrides(context)
//        )
...
//                put(
//                    "message",
//                    "命令已在后台启动，使用 shell_status 查询状态，使用 shell_stop 停止进程"
//                )
//            }.toString(2)
//        }
//
...
//        ALog.d(Component, ALog.event("查询 Shell 进程状态", "id" to id))
...
//        val id = payload.optString("id").trim()
//        require(id.isNotEmpty()) { "id 不能为空。" }
//        ALog.i(Component, ALog.event("请求停止 Shell 进程", "id" to id))
...
//    }
    ...
    private fun scheduleWait(arguments: String): String {
        ...
        ALog.i(Component, ALog.event("工具请求等待", "seconds" to seconds))
        return JSONObject().apply {
            put("ok", true)
        ...
        return JSONObject().apply {
            ...
            put(
                ...
                JSONObject().apply {
                    ...
                    put(
                        "description",
                        "按行读取设备上当前应用有权限访问的文本文件内容。行号从 1 开始。如果不提供行范围参数，默认读取前 200 行。读取中间内容时直接提供 start_line/end_line。"
                    )
                    put(
                        "parameters",
                        JSONObject().apply {
                                ...
                                JSONObject().apply {
                                    ...
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "读取文件前多少行。只在不提供 start_line/end_line 时生效。"
                                            )
                                        }
                                    )
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "起始行号，从 1 开始。读取中间内容时优先使用此字段，可与 end_line 一起使用。"
                                            )
                                        }
                                    )
                                    put(
        ...
        return JSONObject().apply {
            ...
            put(
                ...
                JSONObject().apply {
                    ...
                    put(
                        "description",
                        "获取设备当前本地时间、时区和 UTC 偏移量。用于回答日期、时间、时区或需要准确当前时间的任务。"
                    )
                    put(
                        "parameters",
                        JSONObject().apply {
        ...
        return JSONObject().apply {
            ...
            put(
                ...
                JSONObject().apply {
                    ...
                    put(
                        "description",
                        "通过 HTTP GET 获取一个 http/https URL 的内容。支持 markdown、 raw 原始网页和 text 纯文本。适合读取搜索结果中的网页、文档或接口文本响应。"
                    )
                    put(
                        "parameters",
                        JSONObject().apply {
                                ...
                                JSONObject().apply {
                                    ...
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "enum",
                                                JSONArray().put("text").put("raw").put("markdown")
                                            )
                                        }
                                    )
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "返回 content 的最大字符数，默认 20000，最大 100000。"
                                            )
                                        }
                                    )
                                }
        ...
        return JSONObject().apply {
            ...
            put(
                ...
                JSONObject().apply {
                    ...
                    put(
                        "description",
                        "使用不可见 WebView 加载完整网页，执行页面 JavaScript 后提取渲染后的内容。适合普通 fetch_url 拿不到动态内容时使用；比 fetch_url 更慢且更耗资源。"
                    )
                    put(
                        "parameters",
                        JSONObject().apply {
                                ...
                                JSONObject().apply {
                                    ...
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "WebView 加载超时时间，默认 20 秒，最大 60 秒。"
                                            )
                                        }
                                    )
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "页面 onPageFinished 后继续等待的毫秒数，用于等待异步渲染，默认 1000，最大 10000。"
                                            )
                                        }
                                    )
                                }
        ...
        return JSONObject().apply {
            ...
            put(
                ...
                JSONObject().apply {
                    ...
                    put(
                        "description",
                        "使用 SearXNG 搜索互联网。适合查询最新信息、网页资料、新闻或需要来源链接的问题。"
                    )
                    put(
                        "parameters",
                        JSONObject().apply {
                                ...
                                JSONObject().apply {
                                    ...
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "搜索关键词，使用与用户问题最匹配的自然语言或关键词，最大 300 个字符。"
                                            )
                                        }
                                    )
                                    put(
                                    ...
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "可选的搜索语言代码，例如 zh-CN、en-US 或 all。"
                                            )
                                        }
                                    )
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "可选的分类，多个分类用英文逗号分隔，例如 general、news、it、science。"
                                            )
                                        }
                                    )
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "enum",
                                                JSONArray().put("day").put("week").put("month")
                                                    .put("year")
                                            )
                                        }
                                    )
        ...
        return JSONObject().apply {
            ...
            put(
                ...
                JSONObject().apply {
                    ...
                    put(
                        "description",
                        "使用 Tavily Search 搜索互联网。适合查询最新信息、网页资料、新闻或需要来源链接的问题。"
                    )
                    put(
                        "parameters",
                        JSONObject().apply {
                                ...
                                JSONObject().apply {
                                    ...
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "enum",
                                                JSONArray().put("general").put("news")
                                                    .put("finance")
                                            )
                                        }
                                    )
                                    ...
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "只返回该日期之后发布或更新的结果，格式 YYYY-MM-DD。"
                                            )
                                        }
                                    )
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "只返回该日期之前发布或更新的结果，格式 YYYY-MM-DD。"
                                            )
                                        }
                                    )
                                    put(
                                    ...
                                    put(
                                        ...
                                        JSONObject().apply {
                                            ...
                                            put(
                                                "description",
                                                "advanced 搜索时每个来源返回的内容片段数，1 到 3。"
                                            )
                                        }
                                    )
                                    put(
        ...
        return JSONObject().apply {
            ...
            put(
                ...
                JSONObject().apply {
                    ...
                    put(
                        "description",
                        "使用 AnySearch 搜索互联网。适合查询最新信息、网页资料、新闻或需要来源链接的问题。"
                    )
                    put(
                        "parameters",
                        JSONObject().apply {
...
//                                            put(
//                                                "description",
//                                                "工作目录，不提供则使用 Android/data/包名/files/ai_tmp 作为默认目录"
//                                            )
//                                        }
//                                    )
//                                    put(
...
//                                            put(
//                                                "description",
//                                                "执行高风险命令前由应用内部填充的确认 ID。模型不要自行生成此字段。"
//                                            )
//                                        }
//                                    )
//                                    put(
        ...
        return JSONObject().apply {
            ...
            put(
                ...
                JSONObject().apply {
                    ...
                    put(
                        "description",
                        "等待指定秒数后再继续对话。适用于等待后台 shell 命令执行一段时间后查询其状态。"
                    )
                    put(
                        "parameters",
                        JSONObject().apply {
