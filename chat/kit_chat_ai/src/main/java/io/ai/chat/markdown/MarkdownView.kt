...
import io.ai.chat.R
import io.ai.chat.databinding.AiItemToolResultBinding
import io.ai.chat.helper.IntermediateStepsHelper
import io.ai.chat.model.AssistantMarkdownPart
import io.ai.chat.model.AssistantPart
import io.ai.chat.model.AssistantThinkingPart
import io.ai.chat.model.AssistantToolPart
import io.ai.chat.utils.dp
import io.ai.chat.utils.dpInt
...
class MarkdownView @JvmOverloads constructor(
                ...
                LinearLayout(context).apply {
                    ...
                    addView(LinearLayout(context).apply {
                        ...
                        addView(TextView(context).apply {
                            ...
                            setText(R.string.ai_label_thinking)
                        ...
                        addView(ImageView(context).apply {
                            ...
                            contentDescription = context.getString(
                                if (expanded) R.string.ai_action_collapse else R.string.ai_action_expand
                            )
                            setImageResource(
                                if (expanded) R.drawable.ai_vec_expand_less else R.drawable.ai_vec_expand_more
                            )
                        })
                    })
                    addView(TextView(context).apply {
                    ...
                    val toggle = View.OnClickListener {
                        ...
                        toggleView.contentDescription = context.getString(
                            if (nextExpanded) R.string.ai_action_collapse else R.string.ai_action_expand
                        )
                        toggleView.setImageResource(
                            if (nextExpanded) R.drawable.ai_vec_expand_less else R.drawable.ai_vec_expand_more
                        )
                    }
    ...
    private fun createToolPartView(
        ...
        val binding = AiItemToolResultBinding.inflate(LayoutInflater.from(context), this, false)
        ...
        binding.textSummary.text = if (toolPart.isLoading) {
            context.getString(R.string.ai_tool_summary_loading, summary)
        } else {
            summary
        }
    ...
    private fun toolIconRes(toolName: String): Int {
        return when (toolName) {
            "read_file", "read_local_file" -> R.drawable.ai_vec_description
            "list_path" -> R.drawable.ai_vec_folder_open
            "get_current_time" -> R.drawable.ai_vec_schedule
            "fetch_url" -> R.drawable.ai_vec_link
            "fetch_webview" -> R.drawable.ai_vec_web_asset
            "web_search" -> R.drawable.ai_vec_travel_explore
            "shell" -> R.drawable.ai_vec_terminal
            "shell_status" -> R.drawable.ai_vec_manage_search
            "shell_stop" -> R.drawable.ai_vec_stop_circle
            "wait" -> R.drawable.ai_vec_hourglass_empty
            else -> R.drawable.ai_vec_build
        }
    }
